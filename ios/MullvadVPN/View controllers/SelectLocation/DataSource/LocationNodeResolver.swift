...
final class LocationNodeResolver {
    private let recentProvider: LocationDataSourceProtocol
    private let allProviders: [LocationDataSourceProtocol]

    private var providers: [LocationDataSourceProtocol] { [recentProvider] + allProviders }

    init(
        recentProvider: LocationDataSourceProtocol,
        allProviders: [LocationDataSourceProtocol]
    ) {
    ...
    func setSelectedNodeExpanded(_ isExpanded: Bool) {
        let selectedNode = allProviders.firstNode { provider in
    ...
    func setSelectedNode(selectedRelays: UserSelectedRelays) {
        resetSelection()
        recentProvider.node(by: selectedRelays)?.isSelected = true
        let selectedNode = allProviders.firstNode { provider in
    ...
    func setConnectedRelay(hostname: String?) {
        ...
        let rootNode = RootLocationNode(children: allProviders.flatMap(\.nodes))
        rootNode
            .flattened
            .forEachNode { node in
